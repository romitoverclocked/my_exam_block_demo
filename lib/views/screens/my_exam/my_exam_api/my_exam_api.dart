...
class MyExamApi {
  ...
  static Future<List<ExamData>?> getExamsData(int scanTypeId, int page) async {
    ...
    if (response?.statusCode == 200 && response != null) {
      ...
      List l = map['data']['data'];
      List<ExamData> list = [];
      l.forEach((element) {
        Map json = element as Map;
        list.add(ExamData.fromJson(json));
      });
  ...
  static Future<List<ExamData>?> getSharedWithMeExamData(int scanTypeId) async {
    Response? response = await ApiService.getApi(
